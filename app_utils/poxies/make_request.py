...
from d2dstore.logger import logger
...
class RequestClient(Generic[T]):
    ...
    def __init__(
            ...
            logger: Optional[Callable[[str], None]] = logger,
    ):
    ...
    def request(
        ...
        for attempt in range(1, total_attempts + 1):
            try:
                ...
                if exc.reason:
                    message = f'{message} Reason: {exc.reason}.'
                if exc.code == 403:
                    message = (
                        f'{message} Access forbidden. '
                        'Check authentication/authorization headers, API key, '
        ...
        if last_error.response_body:
            self._log(
                f'Response body: {last_error.response_body[:500]}'
            )
    ...
    def _build_url(
        ...
        if path_params:
            ...
            missing = sorted(
                name for name in placeholders if name not in path_params)
            if missing:
                raise ValueError(
            ...
            for name, value in path_params.items():
                token = f':{name}'
                if token in url:
                    url = url.replace(token, quote(
                        str(value).strip(), safe=''))
