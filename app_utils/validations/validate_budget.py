...
class ValidateBudget:
    ...
    def validate_and_save(self, budget: Budget):
        ...
        # Validate status
        status = [item for item in BUDGET_STATUSES if item[0] ==
                  getattr(self.budget, 'status', self.budget.get('status'))]
        if not status:
        ...
        if len(budgets) > 0:
            raise GraphQLError(
                f"The budget should not take place in the collision date as '{budgets[0]['name']}'")
        ...
        with transaction.atomic():
            ...
            existing_items = {
                itm.id: itm for itm in budget.budget_items.all()} if budget.id else {}
            incoming_ids = set([i.get('id')
                               for i in items_payload if i.get('id')])
    ...
    def validate(self):
        ...
        for idx, item in enumerate(items):
            ...
            try:
                ...
                raise GraphQLError(
                    f"Item '{item.get('name')}' has invalid amount")
            if amount_f < 0:
                raise GraphQLError(
                    f"Item '{item.get('name')}' amount must be non-negative")
    ...
    def edit_items(self, budget: Budget, items: list):
        ...
        for item in items:
            ...
            if bi.budget_id != budget.id:
                raise GraphQLError(
                    f"Item id {item_id} does not belong to this budget")
            # update fields
