...
class ValidateBudget:
	...
	def validate_and_save(self, budget: Budget):
		...
		# Validate status
		status = [item for item in BUDGET_STATUSES if item[0] == getattr(self.budget, 'status', self.budget.get('status'))]
		if not status:
		...
		with transaction.atomic():
			...
			existing_items = {itm.id: itm for itm in budget.budget_items.all()} if budget.id else {}
	...
	def delete_items(self, ids: list):
		...
		for iid in ids:
			...
			# deletion; rely on model delete (may be soft-delete)
			bi.delete()
