    ...
    def mutate(self, info, is_system,  **kwargs):
        ...
        if budget_line:
            if budget_line.active:
                raise GraphQLError(
                    f"The budget line with name '{name}' already exists")
            else:
...
class DeleteBudgetLine(graphene.Mutation):
    ...
    def mutate(self, info, id, is_system=False, **kwargs):
        ...
        budget_filter = Q(d_budget_line__id=id) if is_system else Q(
            u_budget_line__id=id)
...
class EnableBudgetLine(graphene.Mutation):
    ...
    class Arguments:
        id = graphene.String(required=True)
        is_system = graphene.Boolean(required=True)
        is_global_setup = graphene.Boolean(required=False)

    @login_required
    def mutate(self, info, id, is_system=False, is_global_setup=False):
        user = info.context.user
        is_setup = is_global_setup and user.is_superuser
        BudgetLine = DefaultBudgetLine if is_system else UserBudgetLine
        created = False
        if is_setup:
            budget_line = get_model_object(DefaultBudgetLine, 'id', id)
            if not budget_line:
                raise GraphQLError('Budget line not found')
            ...
            if not budget_line:
                raise GraphQLError('Budget line not found')
            if is_system:
                line, created = UserDefaultBudgetLine.objects.get_or_create(
                    user=user, line=budget_line, defaults={'enabled': True})
                if not created:
